#Codigo escrito por yresquirol. Visita https://www.youtube.com/caibnetik https://www.facebook.com/caibnetik
#Leer el archivo README.md para instrucciones.

#SET VARIABLES
:global ifaces 4
:global prefix "wlan"
:global ssid "WIFI_ETECSA"
:global gw 10.202.139.1
:global lanInterface "ether1"
:global lanAddress 192.168.200.1
:global hostToPing 10.219.17.85;

##################### NO CAMBIAR DE AQUI PARA ABAJO ################################
:global rootRoute ("$gw%$prefix" . 1)
for route from=2 to=$ifaces do={ :set $rootRoute ($rootRoute,"$gw%$prefix$route") }

#SET CLIENTS INTERFACES
interface wireless set [ find default-name=wlan1 ] name=("$prefix" . 1) ssid=$ssid disabled=no mode=station frequency-mode=superchannel country=no_country_set channel-width=20mhz wireless-protocol=802.11 station-roaming=disabled
for iface from=2 to=$ifaces  do={ interface wireless add ssid=$ssid master-interface=($prefix . 1) mode=station name="$prefix$iface" disabled=no }
...
#FAILOVER
system script add dont-require-permissions=yes name=Failover source=":local iface \"$prefix$ifaces\";\r\
    \n:local steps 0;\r\
    \n:local tempSteps 1;\r\
...
for code from=1 to=$ifaces do={ system scheduler add interval=5s name=("$prefix$code"."Internet") on-event=("$prefix$code"."Internet") start-time=startup }


#PING ETECSA FOR EACH INTERFACE, IF IT DOESN'T RESPOND THEN RENEW THE IP
system script add dont-require-permissions=no name=renewByPing source="foreach iface in=[interface wireless registration-table find interface~\"$prefix\"] do {\r\
    \n    :local iName [interface wireless registration-table get value-name=interface [find .id=\$iface]];\r\
    \n    do {\r\
    \n        if ([ping 10.180.0.30 count=5 interface=\$iName]=0) do {\r\
    \n            ip dhcp-client renew [find interface=\$iName];\r\
    \n            log warning \"Renovando IP en interfaz \$iName\";\r\
    \n        }\r\
    \n    } on-error={\r\
    \n        # no hacer nada si hay error de enrutamiento\r\
    \n    }\r\
    \n}"
